...
class MLP(LightningModule):
    def __init__(
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features=self.hparams.signal_length, out_features=400), 
...
class SAE(LightningModule):
    def __init__(
        ...
        super().__init__()
        # save parameters to checkpoint
        self.train_ae_idx = train_ae_idx
        self.save_hyperparameters()
        ...
        self.decoder1 = nn.Sequential(
            nn.Linear(in_features=400, out_features=self.hparams.signal_length),
            nn.Dropout(p=0.05), nn.ReLU()
        )
        ...
        # ref: https://stackoverflow.com/a/69778641
        self.params = nn.ModuleDict({
            'ae1': nn.ModuleList([self.encoder1, self.decoder1]),
            'ae2': nn.ModuleList([self.encoder2, self.decoder2]),
            'ae3': nn.ModuleList([self.encoder3, self.decoder3]),
            'ae4': nn.ModuleList([self.encoder4, self.decoder4]),
            'ae5': nn.ModuleList([self.encoder5, self.decoder5]),
            'sae': nn.ModuleList([
                self.encoder1,
                self.encoder2,
                self.encoder3,
    ...
    def configure_optimizers(self):
        if self.train_ae_idx == 1:
            return torch.optim.Adam([{'params': self.params.ae1.parameters()}])
        elif self.train_ae_idx == 2:
            return torch.optim.Adam([{'params': self.params.ae2.parameters()}])
        elif self.train_ae_idx == 3:
            return torch.optim.Adam([{'params': self.params.ae3.parameters()}])
        elif self.train_ae_idx == 4:
            return torch.optim.Adam([{'params': self.params.ae4.parameters()}])
        elif self.train_ae_idx == 5:
            return torch.optim.Adam([{'params': self.params.ae5.parameters()}])
        elif self.train_ae_idx == 0:
            return torch.optim.Adam([{'params': self.params.sae.parameters()}])
